...
Host Client & Mini PC (Intel N5105, 8GB RAM, Ubuntu Server 22.04) \\ \hline
...
\begin{figure}[H]
    ...
    \caption{Demo sistem HMI yang terkoneksi dengan PLC secara \textit{real-time}. Antarmuka menampilkan visualisasi data berupa grafik (\textit{chart}) dan kontrol input berupa \textit{slider} yang responsif terhadap nilai aktual di PLC.}
    \label{fig:demo-run}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Tampilan demo alarm pada HMI ketika suhu oli melebihi batas aman. Sistem menampilkan notifikasi visual dengan tingkat prioritas \textit{High High}.}
...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|}
        ...
        Latensi baca/tulis (rata-rata) & $\leq$ 200 ms & 48 ms & \textcolor{green}{OK} \\ \hline
        ...
        Waktu mulai aplikasi & $\leq$ 10 s & 3.8 s & \textcolor{green}{OK} \\ \hline
    \end{tabular}
...
Pengujian latensi dilakukan dengan mengukur waktu rata-rata yang dibutuhkan PLC untuk merespons permintaan baca dan tulis.  
Hasil pengujian menunjukkan latensi rata-rata sebesar 48 ms, yang masih di bawah batas KPI 200 ms.  

\subsubsection{Persentase Sukses Komunikasi}
Tingkat keberhasilan komunikasi diukur dari jumlah permintaan yang berhasil dijawab dibandingkan dengan total permintaan yang dikirim.  
Dapat dilihat pada gambar~\ref{fig:fins-stress-log}, terdapat 990 permintaan yang sukses dibandingkan dengan 1000 total permintaan. Hasil menunjukkan tingkat keberhasilan 99\%, melampaui KPI minimal 95\%.  

\subsubsection{Penggunaan CPU dan Memori Server}
Selama pengujian, penggunaan CPU server rata-rata berada pada 22\%. Hal ini berarti penggunaan CPU server masih di bawah batas KPI 30\%.
Sedangkan rata-rata penggunaan memori selama uji beban tercatat 280 MB dari total memori sebesar 8 GB. Ini juga berarti bahwa penggunaan memori server masih dalam batas wajar.  

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{images/memori_cpu_usage.jpeg}
    \caption{Monitoring penggunaan memori dan CPU server}
    \label{fig:memory-cpu-usage}
\end{figure}
...
Rata-rata waktu deteksi adalah sekitar 1000 ms, jauh lebih lambat dari batas KPI 200 ms. Hal ini dikarenakan interval terkecil sistem HMI untuk mendeteksi situasi dan kemudian mengirimkan alarm adalah 1 detik seperti yang ditunjukkan pada gambar~\ref{fig:alarm-interval}. 
...
Rata-rata waktu mulai aplikasi adalah 3,8 detik, memenuhi KPI $\leq$ 10 detik.
